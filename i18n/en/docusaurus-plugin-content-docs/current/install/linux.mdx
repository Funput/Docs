...
:::info
Only `--no-repo` and `--version` download from **GitHub Releases**; those two paths have **no automatic update** — run the script again to upgrade.
:::

## Arch Linux (AUR)

Arch has no package on GitHub Releases or `repo.funput.app`. Install via an AUR helper (e.g. `yay`) — the packages build from the release tag's source:
...
Both pull in `funput-settings` (the Settings app) at exactly the same version. Updates come through your AUR helper like any other AUR package.

:::tip
No AUR helper, or no sudo? The user-local install works on Arch too — it only needs an IBus or Fcitx5 daemon already present:
...
| AUR | Through your AUR helper (`yay -Syu`) |

Automatic updates through apt/dnf/zypper require the **repo.funput.app** repository — added by hand as in Option 1, or by `install.sh` for you.
