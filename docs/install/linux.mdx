...
:::caution
Gói dựng sẵn cho **x86_64**. Arch Linux ARM là distro riêng, không dùng được kho này — trên máy ARM hãy dùng [đường `--user`](#không-sudo-trên-arch).
:::
...
:::info
Đây là gói **binary**, link vào thư viện Arch tại thời điểm dựng. Arch cập nhật liên tục, nên sau một đợt bump soname (fcitx5, ibus, gtk4) gói có thể tạm không nạp được. Kho **tự dựng lại hàng tuần** trên Arch mới nhất, nên tình trạng đó được vá trong vòng một tuần mà không cần đợi Funput ra bản mới — `pacman -Syu` sẽ nhận bản dựng lại đó. Muốn build từ mã nguồn ngay trên máy mình thì dùng AUR khi kênh đó mở.
:::

### Không sudo trên Arch

Đường cài vào `~/.local` chạy được trên mọi kiến trúc, chỉ cần đã có sẵn daemon IBus hoặc Fcitx5:
